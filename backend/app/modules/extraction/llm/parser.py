...
def _strip_code_fences(text: str) -> str:
    """Remove enclosing markdown code fences (`````json```` … ````````).

    Handles:
      - `` ```json ``…`` ``` ``
      - `` ``` ``…`` ``` ``
      - `` ```python ``…`` ``` `` (strips any language identifier)
      - Variants with leading whitespace
    """
...
def _find_json_object(text: str, start: int = 0) -> str | None:
    ...
    Returns the substring if a balanced object is found, or ``None``.
    """
...
def parse_llm_json(
    ...
    2. Remove enclosing markdown code fences (`` ```json`` …`` ``` ``).
    3. Try ``json.loads()`` directly on the sanitised result.
    4. If that fails, search for a balanced ``{``…``}`` or ``[``…``]``
       and attempt to parse only that substring.
    5. If nothing works, raise ``LLMParsingError`` with full diagnostics.

    Parameters
    ----------
    raw:
        The raw response content from the LLM provider.
    finish_reason:
        Optional ``finish_reason`` from the provider (included in
        error diagnostics).
    ...
    ------
    LLMParsingError
        If the response cannot be parsed after all sanitisation steps.
    """
    stripped = raw.strip()
    ...
    for label, candidate in [
        ("after-fence-strip", sanitized),
        ("after-fence-strip-trimmed", sanitized.strip()),
    ]:
        try:
    ...
    for label, extractor in [("object", _find_json_object), ("array", _find_json_array)]:
    ...
    raise LLMParsingError(
        message=(
            ...
            f"Use DEBUG logging for full content."
        ),
        raw=raw,
        sanitized=sanitized,
        finish_reason=finish_reason,

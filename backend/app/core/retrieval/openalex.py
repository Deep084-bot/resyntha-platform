...
Returns normalised ``RetrievalResult`` with canonical ``Paper`` objects.
"""
...
class OpenAlexProvider(BaseRetrievalProvider):
    ...
    METADATA = ProviderMetadata(
        ...
        confidence=0.9,
        supported_domains=["cs", "physics", "math", "biology", "medicine", "chemistry"],
        coverage="Broad multidisciplinary coverage of scholarly works",
    ...
    async def health_check(self) -> bool:
        """Check OpenAlex API reachability with a minimal query."""
        try:
    ...
    async def search(self, query: str, limit: int = 10) -> RetrievalResult:
        ...
        try:
            ...
            raise RetrievalAPIError(
                f"OpenAlex HTTP error {exc.response.status_code}",
                response_time_ms=round(elapsed, 2),
            ) from exc
        except httpx.ConnectError as exc:
            elapsed = (time.monotonic() - start) * 1000
            logger.error("openalex_search_connection_failed", query=query, error=str(exc))
            ...
            elapsed = (time.monotonic() - start) * 1000
            logger.error("openalex_search_timeout", query=query, error=str(exc))

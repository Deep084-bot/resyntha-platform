"""Alembic environment configuration.

Reads the database URL from the application Settings so that the same
connection string is shared between the runtime and migration tooling.
"""

from logging.config import fileConfig

from alembic import context
from sqlalchemy import engine_from_config, pool

from app.config import get_settings
from app.database.base import Base

# Import every ORM model so that Base.metadata is fully populated before
# Alembic reads it.  Without this, autogenerate would produce empty
# migrations because no tables would be registered.
import app.database.model_registry  # noqa: F401

# Alembic Config object, which provides access to values within .ini file.
config = context.config

# Override ini-file URL with the application setting so there is a single
# source of truth for the connection string.
settings = get_settings()
config.set_main_option("sqlalchemy.url", settings.DATABASE_URL)

# Set up Python logging from the ini-file section.
if config.config_file_name is not None:
    fileConfig(config.config_file_name)

# Metadata for autogenerate support — Alembic compares this against the
# live database to produce migration scripts.
target_metadata = Base.metadata


def run_migrations_offline() -> None:
    """Run migrations in 'offline' mode.

    Configures the context with just the URL and not an Engine, so
    migrations are written as SQL scripts without a live connection.
    """
    url = config.get_main_option("sqlalchemy.url")
    context.configure(
        url=url,
        target_metadata=target_metadata,
        literal_binds=True,
        dialect_opts={"paramstyle": "named"},
    )

    with context.begin_transaction():
        context.run_migrations()


def run_migrations_online() -> None:
    """Run migrations in 'online' mode.

    Creates an Engine from the config and associates a connection with
    the migration context.
    """
    connectable = engine_from_config(
        config.get_section(config.config_ini_section, {}),
        prefix="sqlalchemy.",
        poolclass=pool.NullPool,
    )

    with connectable.connect() as connection:
        context.configure(
            connection=connection,
            target_metadata=target_metadata,
        )

        with context.begin_transaction():
            context.run_migrations()


if context.is_offline_mode():
    run_migrations_offline()
else:
    run_migrations_online()
